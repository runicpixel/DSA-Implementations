...
namespace Heap
{
    public class Heap<K, D> where K : IComparable<K>
    {
        ...
        public IHeapifyable<K, D> Delete()
        {
            if(Count == 0)
            {
                throw new InvalidOperationException("nothig is there to delete");
            }
        ...
        private void DownHeap(int x)
        {
            while(2*x <= Count)
            {
                ...
                int MainChild= LeftChild;

                if(RightChild <= Count)
                {
                    if(comparer.Compare(data[RightChild].Key, data[LeftChild].Key) <0)
                    {
                        MainChild = RightChild;
                    }
                }

                if (comparer.Compare(data[MainChild].Key, data[x].Key) >= 0)
                {
                    return;
                }
        ...
        public IHeapifyable<K, D>[] BuildHeap(K[] keys, D[] data)
        {
            ...
            {
                throw new
                InvalidOperationException("heap already has element");
            }

            int min = Math.Min(data.Length,keys.Length);
        ...
        {
            Node one = element as Node;

            if(!one.Equals(data[one.Position]))
            {
                throw new
                InvalidOperationException("can't be given state of heap");
        ...
        {
            Node del = element as Node;
            if(Count == 0)
            {
                throw new
                InvalidOperationException("empty heap");
            }

            Node last = data[del.Position];

            Swap(del.Position,Count); 

            Count--;

            if(comparer.Compare(data[del.Position].Key, data[del.Position/2].Key) < 0)
            {
                UpHeap(del.Position);
            }
        ...
        public IHeapifyable<K, D> KthMinElement(int k)
        {
            ...
            for(int i =0; i<k; i++)
            {
                Node k_element = Delete() as Node;
                temp[i] = k_element;
            }

            for(int j = 0; j < k; j++)
            {
                this.Insert(temp[j].Key,temp[j].Data);
            }
