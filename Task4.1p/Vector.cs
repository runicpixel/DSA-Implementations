...
namespace Vector
{
    public class Vector<T> : IEnumerable<T> where T : IComparable<T>
    {
        ...
        {
            return BinarySearch(element, 0, data.Length - 1, comparer);
        }
        ...
        private int BinarySearch(T element, int left, int right, IComparer<T> comparer)
        {
            ...
            {
                return BinarySearch(element, middle + 1, right, comparer);
            }
            else
            {
                return BinarySearch(element, left, middle - 1, comparer);
            }
        }
        ...
        private class VectorIterator : IEnumerator<T>
        {
            ...
            {
                currentIndex++;
                return currentIndex < vector.Count;
            }
